...
void FightersSystem::resetFighterPositions() {
	...
	f0Tr->position_ = Vector2D(10, game_->getWindowHeight() / 2.0 - 25);
	f0Tr->rotation_ = 90.0;
...
void FightersSystem::recieve(const msg::Message& msg)
{
	...
	{
		if (msg.senderClientId != game_->getNetworking()->getClientId())
		{
			ecs::HdlrId other;
			if (game_->getNetworking()->getClientId() == 0)
			{
				other = ecs::_hdlr_Fighter1;
			}
			else
				other = ecs::_hdlr_Fighter0;
			Transform* tr = mngr_->getHandler(other)->getComponent<Transform>(ecs::Transform);
			const FighterInfoMsg& message = (const FighterInfoMsg&)msg;
			tr->position_ = {message.x, message.y};
			tr->rotation_  = message.rot;
			break;
		}
	}
...
void FightersSystem::updateFighter(Entity *e) {
	...
	if (ih->keyDownEvent()) {
		...
		else if(ih->isKeyDown(keys->left) || ih->isKeyDown(keys->right) || ih->isKeyDown(keys->speedup) || ih->isKeyDown(keys->slowdown))
		{
	...
	//sends new position
	mngr_->send<msg::FighterInfoMsg>(tr->position_.getX(),
		tr->position_.getY(), tr->rotation_);
}
